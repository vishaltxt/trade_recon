import { Interactive as XTSInteractive } from "xts-interactive-api";
...
export const initXTS = async () => {
  xts = new XTSInteractive({
  ...
  try {
    const response = await xts.logIn();
    sessionToken = response.result.token;
    console.log("✅ XTS Login successful, token:", sessionToken);
  } catch (err) {
    console.error("❌ XTS Login failed:", err.message);
  }
};

export const placeOrder = async (orderData) => {
  if (!xts) {
    throw new Error("XTS not initialized");
  }

  try {
    const response = await xts.placeOrder(orderData);
    return response;
  } catch (err) {
...
// services/example/interactiveTestApi.js
// import { Interactive as XTSInteractive } from "xts-interactive-api";
// import dotenv from "dotenv";
// dotenv.config();
...
// export const initXTS = async () => {
//   xts = new XTSInteractive("https://developers.symphonyfintech.in"); // ✅ pass base URL

//   const loginRequest = {
//     secretKey: process.env.XTS_SECRET_KEY,
//     appKey: process.env.XTS_API_KEY,
//     source: "WEBAPI",
//   };

//   try {
//     const response = await xts.logIn(loginRequest);
//     sessionToken = response?.result?.token;
//     console.log("✅ XTS Login successful, token:", sessionToken);
//   } catch (err) {
//     console.error("❌ XTS Login failed:", err.message);
...
//   try {
//     console.log("🚀 Sending order to XTS:", orderData);
//     const response = await xts.placeOrder(orderData);
//     return response;
//   } catch (err) {
//     throw new Error(err.message);
...
// import { Interactive as XTSInteractive, WS as XTSInteractiveWS } from 'xts-interactive-api';
...
//   } else {
//     //In case of failure
//     console.error("logIn error in api",logIn);
//   }
// })();
...
// export var placeOrder = async function (placeOrderRequest) {
